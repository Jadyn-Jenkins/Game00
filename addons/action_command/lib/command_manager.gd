...
@export var max_cache_frame: int = 30
...
class MatchResult:
	...
	func use(type: int = UseType.Last):
...
func _action_data(name, state) -> ActionData:
	var action_data;
	if _cached_action_data.is_empty():
...
class FrameMatcher:
	...
	func _to_string():
		var arr = PackedStringArray(matchers)
		return '&'.join(arr)
...
func command_match(queue: Array) -> MatchResult:
	...
	while true:
		...
		if ret != null:
			...
			c_queue.pop_back()
		frame_count+=1
		if frame_count >= _frames.size() and not c_queue.is_empty():
			return null
	return null
