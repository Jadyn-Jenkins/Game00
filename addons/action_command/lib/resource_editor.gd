...
func _reload_buttons():
	...
	for btn in target.buttons:
		var btn_editor = KeyEditor.instantiate()
		btn_editor.set_action(btn)
		btn_editor.index = count
		btn_editor.connect("button_pressed", Callable(self, "_on_button_editor_pressed"))
...
func _on_add_button_pressed():
	...
	await key_input_dialog.input_key(session).completed
