...
func build_font():
	...
	for i in _texture_mappings.size():
		...
		super.add_texture(mapping.scaled_texture)
	...
	if not is_space_defined and not fallback and _texture_mappings.size() > 0:
		...
		var extra_space := 0
		if mono:
			extra_space = _texture_mappings[0].size.x * _texture_mappings[0].scale
