...
@export var scaled_texture: Texture2D
...
@export var scale := 1: get = get_scale, set = set_scale
...
@export var interpolation := Image.INTERPOLATE_NEAREST: get = get_interpolation, set = set_interpolation
...
@export var chars: String: get = get_chars, set = set_chars
...
func get_scaled_texture() -> Texture2D:
	if scale == 1:
	...
	if scaled.x > 16384 or scaled.y > 16384:
		push_error("Could not Upscale Texture2D! Texture2D too large")
		return source_texture
...
func _get_char_rect_unscaled(char_code: int):
	...
	var rect := Rect2(
		(size + rect_gap) * pos + texture_offset,
		size
	)
...
func _scan_empty_pixels(from_x: int, to_x: int, from_y: int, to_y: int) -> int:
	...
	false # img_data.lock() # TODOConverter40, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	var c = 0
	...
	for x in range(from_x, to_x, dir):
		for y in range(from_y, to_y):
			...
			if pix.a != 0.0:
				return c
		c += 1
	false # img_data.unlock() # TODOConverter40, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	
	return c


func get_local_position(position: Vector2) -> Vector2:
	var pos = ((position - texture_offset * scale) / ((size + rect_gap) * scale)).floor()
	return pos
...
func get_rect_for_position(position: Vector2):
	...
	var pos = get_local_position(position)
	return Rect2(
		((size + rect_gap) * pos + texture_offset) * scale,
		size * scale
	)
