...
func _ready():
	file_settings.connect("change", Callable(self, "queue_save"))
	font_settings.connect("change", Callable(self, "queue_save"))
...
func _save():
	if font_ref and font_ref.get_ref():
		...
		if error != OK:
			push_error("Failed to Save Font with Path3D: " + font.resource_path + ". Error Code: " + String(error))
		else:
...
func _add_texture_ui(texture: Texture2D, idx := -1):
	var file_node := file_scene.instantiate()
	...
	file_node.connect("file_removed", Callable(self, "_on_file_removed"))
	file_node.connect("file_changed", Callable(self, "_on_file_changed"))
